...
void Mesh_Pipeline::Add_Mesh_to_Pipeline(std::string filename, Vec3d origin){
    ...
    std::shared_ptr<Triangle_Modifier> this_mod(new Translator(origin.x, origin.getY(), origin.getZ()));
...
void Mesh_Pipeline::Add_OBJ_Mesh_to_Pipeline(std::string filename, Vec3d origin, Vec3d rotation){
    ...
    for (auto new_mesh:OBJ_meshes){
        ...
        std::shared_ptr<Triangle_Modifier> this_rotate_mod(new Rotator(rotation.x, rotation.getY(), rotation.getZ(), Vec3d(0,0,0)));
        std::shared_ptr<Triangle_Modifier> this_translate_mod(new Translator(origin.x, origin.getY(), origin.getZ()));
